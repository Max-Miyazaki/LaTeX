...
この式は, 運動量が明確に定義された1粒子状態の線形重ね合わせで, 係数 $\dfrac{1}{2E_{\mathbf{p}}}$ を除けば位置固有状態 $|\mathbf{x}\rangle$ に対応する非相対論的な波動関数の表現と同じである. よって, 我々は同じ解釈を採用して演算子 $\phi(\mathbf{x})$ は真空状態に作用することで位置 $\mathbf{x}$ に粒子を生成する演算子という解釈を持つ. この解釈は以下の計算によって裏付けられる：
