...
\begin{equation*}\label{2-3.g1}
    \phi(\mathbf{x}, t) = \int \frac{d^3p}{(2\pi)^3} \left( f_{\mathbf{p}}(t) e^{i\mathbf{p}\cdot\mathbf{x}} + f_{\mathbf{p}}^*(t) e^{-i\mathbf{p}\cdot\mathbf{x}} \right) \tag{2-3.g1}
\end{equation*}
ここで $f_{\mathbf{p}}(t)$ は時間に依存するモード成分であり, 明確な形は決まってない.\\
次に Klein-Gordon 方程式に \eqref{2-3.g1} を代入すると,
\begin{align*}
    &(\partial_t^2 - \nabla^2 + m^2) \phi(\mathbf{x}, t) = 0 \tag{2-3.g2} \\
    &(\partial_t^2 - \nabla^2 + m^2) \int \frac{d^3p}{(2\pi)^3} \left( f_{\mathbf{p}}(t) e^{i\mathbf{p}\cdot\mathbf{x}} + f_{\mathbf{p}}^*(t) e^{-i\mathbf{p}\cdot\mathbf{x}} \right) = 0 \tag{2-3.g3}\\
    &\int \frac{d^3p}{(2\pi)^3} \left[ \left( \ddot{f}_{\mathbf{p}}(t) + \mathbf{p}^2 f_{\mathbf{p}}(t) + m^2 f_{\mathbf{p}}(t) \right) e^{i\mathbf{p}\cdot\mathbf{x}} + \left( \ddot{f}_{\mathbf{p}}^*(t) + \mathbf{p}^2 f_{\mathbf{p}}^*(t) + m^2 f_{\mathbf{p}}^*(t) \right) e^{-i\mathbf{p}\cdot\mathbf{x}} \right] = 0 \tag{2-3.g4}
\end{align*}
指数関数 $e^{i\mathbf{p}\cdot\mathbf{x}}$ は各 $\mathbf{p}$ ごとに異なる関数であり, 互いに重ねて表すことができない(線型独立). (※$e^{i\mathbf{p}\cdot\mathbf{x}}$ が線型独立であるから, 各 $\mathbf{p}$ ごとに独立した運動方程式を持ち, 各モードが独立した自由度(調和振動子)として量子化できる構造となっている.)
\vskip\baselineskip
よって, それぞれの係数がゼロになる必要がある：
\begin{equation*}
